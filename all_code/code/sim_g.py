...
def gen_rnd_sample_2(n_p,n_s,mu,sig2_p,sig2_s,sig2_ps):
    sd_p,sd_s,sd_ps = np.sqrt(sig2_p),np.sqrt(sig2_s),np.sqrt(sig2_ps) 
    z_p,z_s,z_ps = np.random.standard_normal(size=(n_p,1)),np.random.standard_normal(size=(1,n_s)),np.random.standard_normal(size=(n_p,n_s))
...
def gen_rnd_sample_3(n_p,n_s,n_d,mu,sig2_p,sig2_s,sig2_d,sig2_ps,sig2_pd,sig2_ds,sig2_pds):
    sd_p,sd_s,sd_d,sd_ps,sd_pd,sd_ds,sd_pds = np.sqrt(sig2_p),np.sqrt(sig2_s),np.sqrt(sig2_d),np.sqrt(sig2_ps),np.sqrt(sig2_pd),np.sqrt(sig2_ds),np.sqrt(sig2_pds)
...
def mc_experiment(N,obs_vec = np.array([500]),facets=2,type='bi'):
    ...
    df = pd.DataFrame(data=sim_res)
    if (type == 'bi'):
        df.to_excel("output_di_"+str(facets)+'_'+str(np.random.rand(1)[0])+".xlsx")  
    elif (type=='norm'):
        df.to_excel("output_norm_"+str(facets)+'_'+str(np.random.rand(1)[0])+".xlsx")  
...
mc_experiment(N=1,facets=3,type='bi')
