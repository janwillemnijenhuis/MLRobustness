...
def run_sims2(N,n_p,n_s=10,sig2 = np.array([0,4,8,2])):
...
def run_sims_di_2(N,n_p=1000,n_s=10,sig2 = np.array([0,4,8,2])):
    sig2 = normalize(sig2)
    sim_res = np.zeros((N,3))
    for i in tqdm(range(N)):
...
def mc_experiment(N,obs_vec = np.array([250,500,1000,2000]),facets=2,type='bi'):
    ...
    for i in tqdm(range(len(obs_vec))):
        ...
        if (type=='bi'):
            if (facets==2):
                sim_res = run_sims_di_2(N,n_p=n_p,n_s=10)
            elif (facets==3):
                sim_res = run_sims_di_3(N,n_p=n_p,n_s=10,n_d=10)
            ...
            if (facets==2):
                sim_res = run_sims2(N,n_p=n_p,n_s=10)
            elif (facets==3):
                sim_res = run_sims(N,n_p=n_p,n_s=10,n_d=10)
            data[row,:] = np.mean(sim_res,axis=0)
            data[row+incr+1,:] = np.mean(np.abs(data-true),axis=0)
            data[row+2*incr+2,:] = np.std(data,axis=0)
            data[row+3*incr+3,:] = rmse(data,true)
...
mc_experiment(N=1000,facets=2,type='bi')